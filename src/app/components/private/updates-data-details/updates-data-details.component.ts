...
@Component({
  selector: 'app-updates-data-details',
  templateUrl: './updates-data-details.component.html',
  styleUrl: './updates-data-details.component.scss',
})
export class UpdatesDataDetailsComponent implements OnInit {
  ...
  /** Detalle de novedad calidad de datos (tabla gestor.novedad_calidad_datos_detalle). */
  novedadDetalle?: NovedadCalidadDatosDetalle;
  ...
  /** Formulario para datos de Genesys (columna izquierda, editable). */
  comparisonForm: FormGroup;

  /** Datos de Registraduría (columna derecha, solo lectura). Cargar desde tu API cuando la tengas. */
  datosRegistraduria: {
    primerNombre: string;
...
/** Carga el detalle de la novedad de calidad de datos llamando al endpoint por id_solicitud y actualiza el formulario y la vista. */
  ...
  private patchFromNovedadDetalle(n: NovedadCalidadDatosDetalle): void {
    ...
    this.comparisonForm.patchValue({
      primerNombre: n.primer_nombre_genesys ?? '',
      segundoNombre: n.segundo_nombre_genesys ?? '',
      primerApellido: n.primer_apellido_genesys ?? '',
    ...
    this.datosRegistraduria = {
      ...
      segundoNombre: n.segundo_nombre_reg ?? '',
      primerApellido: n.primer_apellido_reg ?? '',
  ...
  /** Rellena el formulario Genesys con los datos del afiliado (trabajador.persona). */
  private patchComparisonFormFromPersona(): void {
    ...
    this.comparisonForm.patchValue({
      ...
      fechaExpedicionDoc: toDateInput(p.fecha_expedicion_doc),
      fechaNacimiento: toDateInput(p.fecha_nacimiento),
    });
  }

  /** Carga datos de Registraduría. Sustituir por llamada a tu API cuando exista. */
  loadDatosRegistraduria(): void {
    const p = this.afiliationRequestDetails?.trabajador?.persona;
    if (p) {
      this.datosRegistraduria = {
        primerNombre: p.primer_nombre ?? '',
  ...
  tieneDiferencias(): boolean {
    const fields = ['primerNombre', 'segundoNombre', 'primerApellido', 'segundoApellido', 'fechaExpedicionDoc', 'fechaNacimiento'] as const;
    return fields.some(f => this.isFieldDifferent(f));
  }

  /** Navega de vuelta a la vista de consulta de novedades de calidad de datos. */
  regresar(): void {
    this.router.navigate([RoutesApp.SEARCH_UPDATES_DATA]);
  }
